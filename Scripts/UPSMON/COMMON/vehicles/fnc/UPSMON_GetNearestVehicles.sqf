...

{
	...
	if (_isuav != 1) then
	{
		...
		If ("transport" in _types && _Cargocount > 0) then {_points = _points + 10*_Cargocount};
		If ("gun" in _types && _Gunnercount > 0) then {_points = _points + 50*_gunnercount};
		...
		If (getposATL _x select 2 <= 0.5) then
		{
			...
			{
					//Only take vehicles which do not have driver yet
						If (_driverCount > 0 and {
							("gun" in _types and {_gunnercount > 0}) or
							("tranport" in _types and {_cargoCount > 0})
							}) then
						{

...
//Laste edited 20 09 19 by Kortonki: optimisation and edits to make sure position is found
...
_isMan = false;
_dist = 1;
_mindist = 5; //this changed from 10 to 5: to avoid not finding positions
_water = 0;
...
while {_scan} do
{
		...
		If (("car" in _typeofgrp) || ("tank" in _typeofgrp) || ((group _npc) getvariable ["UPSMON_movetolanding",false])) then
		{
			if (!(surfaceIsWater _targetPosTemp)) then
			{
				...
				If ((_terrainscan select 0) == "meadow" || (_terrainscan select 0) == "forest") then
				{
					...
					{
						//Changed mindistance from 50 to 15
						If (count (_targetPosTemp isflatempty [15,-1,10,10,0,false,objnull]) > 0) then
						{
	...
	If (count _targetPos > 0 || _i > 20) then {_scan = false}; //Changed from 70 to 20 (optimisation)
};

If (count _targetPos == 0) then {_targetPos = _centerpos;}; //this changed from _currpos to _centerPos to avoid stuck group
_targetPos;

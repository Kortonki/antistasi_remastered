...
private _pos = [_position select 0, _position select 1, 2];
...
while {sleep AS_spawnLoopTime; _location call AS_location_fnc_spawned and {!(_location in ([] call AS_location_fnc_knownLocations))}} do {
  {
    ...
    //Only reveal to location if there's radio coverage and the leader is alive after the contact for some.
      ...
      if (_x distance2D _position < 100) then {
        ...
        if (_leader == _x and {(position _x) call AS_fnc_hasRadioCoverage and {!(_x getVariable ["revealing", false]) and {([_x, "VIEW"] checkVisibility [eyepos _x, _pos]) > 0.5}}}) then {

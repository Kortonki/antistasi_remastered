...
private _fnc_spawn = {
	...
	private _apcs = ["NATO", "apcs"] call AS_fnc_getEntity;
	{
		call {
			...
			if (typeOf _x in (["NATO", "static_mg"] call AS_fnc_getEntity)) exitWith {_turretArray pushBack _x;};
			if (typeOf _x in (["NATO", "static_aa"] call AS_fnc_getEntity)) exitWith {_turretArray pushBack _x;};
	...
	private _gunnerType = ["NATO", "gunner"] call AS_fnc_getEntity;
	{
		...
		if (AS_P("NATOsupport") < 50 and (typeOf _x in (["NATO", "static_aa"] call AS_fnc_getEntity))) then {

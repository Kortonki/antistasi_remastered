...
if (_killer call AS_fnc_getSide in ["FIA", "NATO"]) then {
  ...
  if (_type in (["CSAT", "self_aa"] call AS_fnc_getEntity)) then {_effect = 5};

  [0, -(_effect)] remoteExec ["AS_fnc_changeForeignSupport", 2];
  if (alive _veh) then { //Capturing nets more support
    [-(_effect/2),(_effect/2), position _veh] remoteExec ["AS_fnc_changeCitySupport",2];
  } else {
    [-(_effect/2),0, position _veh] remoteExec ["AS_fnc_changeCitySupport",2];
  };

  if (_type in ((["CSAT", "helis_transport"] call AS_fnc_getEntity) + (["CSAT", "helis_attack"] call AS_fnc_getEntity) + (["CSAT", "helis_armed"] call AS_fnc_getEntity) + (["CSAT", "planes"] call AS_fnc_getEntity) + (["CSAT", "self_aa"] call AS_fnc_getEntity)) and {_killer call AS_fnc_getSide == "FIA"}) then {

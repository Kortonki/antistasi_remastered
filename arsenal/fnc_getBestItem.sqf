...
if (_type == "binoculars") then {
	...
	_allItemsAttrs = AS_allBinocularsAttrs;
	_unlockedItems = unlockedWeapons;
...
if (_type == "throw_grenades") then {
	...
	if (_type == "throw_smokes") then {
		...
		_allItems = [];
		{
			private _attr = AS_allThrowSmokesAttrs select _foreachindex;
			if ((_attr select 0) > 0) then {
				_allItemsAttrs pushBack _attr;
				_allItems pushBack _x;
			}
		} forEach AS_allThrowSmokes;
		_unlockedItems = unlockedMagazines;

		_sortingFunction = {
			private _index = _input0 find _x;
			private _mass = (_input1 select _index) select 0;
			private _life = (_input1 select _index) select 1;
			private _amount = (_input1 select _index) select 2;

			private _w_factor = 1.0/(1 + exp (-2*(_amount - 5)));  // 0 => 0; 5 => 0.5; 10 => 1

			//Lighter smokes are considered better

			_w_factor*(1/_mass)*_life
			};
		};

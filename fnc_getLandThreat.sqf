...
// roadblocks
_threat = _threat + 2 * (
	{(_x call AS_location_fnc_position) distance _position < AS_P("spawnDistance")} count ([["roadblock", "watchpost", "fia_hq"], "FIA"] call AS_location_fnc_TS));
...

{
	...
	if !(_type isEqualTo "WeaponHolderSimulated") then {
		...
		call {
		  ...
		  if (_veh in AS_allATStatics) exitWith {_threat = _threat + 2;};
		  if (_veh in AS_allMGStatics) exitWith {_threat = _threat + 1;};
		  if (_veh in AS_allMortarStatics) exitWith {_threat = _threat + 1;};

...
if (_veh distance (getMarkerPos "FIA_HQ") > 200) exitWith {hint "The vehicle must be closer than 200 meters to the flag"};
...
if (_veh call AS_fuel_fnc_getFuelCargoSize > 0) then {
	_fuel = _fuel + (_veh getVariable ["fuelCargo", 0]);
};
[_fuel] remoteExec ["AS_fuel_fnc_changeFIAfuelReserves", 2];

//any boxes attached will be let loose
{
	detach _x;

...
if not (_prestige IsEqualTo [0, 0]) then {
    if (_prestige select 0 != 0) then {
        _description pushBack (format ["%1 support: %2", (["NATO", "shortname"] call AS_fnc_getEntity), _prestige select 0]);
    };
    if (_prestige select 1 != 0) then {
        _description pushBack (format ["%1 support: %2", ["CSAT", "shortname"] call AS_fnc_getEntity, _prestige select 1]);
    };
};
...
if not (_changeAAFattack == 0) then {
    _description pushBack (format ["Disruption in %1 organization: %2", (["AAF", "shortname"] call AS_fnc_getEntity), _changeAAFattack/60]);
};

...
private _fnc_wait_to_load = {
	...
	[_truck, _bankPosition, 120, _fnc_loadCondition, _fnc_missionFailedCondition, _str_unloadStopped] call AS_fnc_wait_or_fail;
...
private _fnc_wait_to_return = {
	...
	if (call _fnc_missionFailedCondition) then {
		([_mission, "FAILED"] call AS_mission_spawn_fnc_loadTask) call BIS_fnc_setTask;
		[_mission, [_bankPosition]] remoteExec ["AS_mission_fnc_fail", 2];
	} else {
		([_mission, "SUCCEEDED"] call AS_mission_spawn_fnc_loadTask) call BIS_fnc_setTask;
		[_mission, [_bankPosition]] remoteExec ["AS_mission_fnc_success", 2];
	};

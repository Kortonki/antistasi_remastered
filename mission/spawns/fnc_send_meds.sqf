...
private _fnc_wait = {
	...
	waitUntil {sleep 1; ((_crate distance _position < 40) and (speed _crate < 1)) or _fnc_missionFailedCondition};
...
private _fnc_run = {
	...
	private _fnc_unloadCondition = {
		...
		(_crate distance _position < 40) and {not(_crate getVariable "asCargo")} and
		{{alive _x and not (_x call AS_medical_fnc_isUnconscious)} count ([40, _crate, "BLUFORSpawn"] call AS_fnc_unitsAtDistance) > 0} and
		{{!(side _x in [("FIA" call AS_fnc_getFactionSide), civilian]) and {_x distance _crate < 40} and {!(_x call AS_medical_fnc_isUnconscious)}} count allUnits == 0}
	};
